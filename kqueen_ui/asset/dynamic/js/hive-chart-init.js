...
var K8SVisualisations = (K8SVisualisations = {}) => {
  ...
  K8SVisualisations.hiveChart.init = (selector, data, config = {}) => {
    if (!data) {
    ...
    var width = config.width || 'auto',
      ...
      createLinks = (nodes, relations) => {
        return relations.map((link) => {
          var retLink = {};
          nodes.forEach((node) => {
      ...
      var nodes = createNodes(data.items),
          links = createLinks(nodes, data.relations);
      ...
      var icon = (i) => icon_mapping[i];
      var color = (i) => color_mapping[i];
      ...
      function render() {
        ...
        var svg = container
          ...
          .attr(
            'transform', 
            'translate(' + (targetWidth / 2 - 80) + ',' + (targetHeight / 2 - 20) + ')'
          );
        var axe = svg
          .selectAll('.node')
          .data(axes)
        ...
        axe
          ...
          .attr('x1', (d) => radius_mapping[d.kind].range()[0])
          .attr('x2', (d) => radius_mapping[d.kind].range()[1]);
          ...
          axe
            ...
            var mouseFunctions = {
              ...
              nodeOver: (d) => {
                svg.selectAll('.link').classed('active', (p) => p.source === d || p.target === d);
                d3.select(this).select('circle').classed('active', true);
                d3.select(this).select('text').classed('active', true);
                tooltip
                  .html('Node - ' + d.name + '<br/>' + 'Kind - ' + d.kind)
            ...
            svg
              ...
              //.style('stroke', (d) => color(d.source.kind))
            ...
            var node = svg
              ...
              .attr('class', 'node')
              .attr('transform', (d) => {
                var radius = radius_mapping[d.kind](d.y * itemStep[d.kind] - 0.1); 
                var x = radius * Math.cos(Math.radians(angle(d)));
                var y = radius * Math.sin(Math.radians(angle(d)));
                return 'translate(' + x + ', ' + y + ')';
              })
            ...
            node.append('use').attr('xlink:href', (d) => icon(d.kind));
        }
        render();
